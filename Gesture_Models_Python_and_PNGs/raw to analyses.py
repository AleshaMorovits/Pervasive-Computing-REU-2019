...
for xy in range(32):
    ...
    for a in range(len(result)): #REMOVE OTHER USERS
        if (xy != int(result[a][len(result[0])-2])):
            indexes.append(a)
    ...
    for z in range (len(X)):
        RangeX.append(X[z][3] - X[z][0])
        RangeY.append(X[z][4] - X[z][1])
        RangeZ.append(X[z][5] - X[z][2])
        
    #X = np.column_stack((X,RangeX))
    #X = np.column_stack((X,RangeY))
    #X = np.column_stack((X,RangeZ))
    
    subset = [0, 1, 2, 3, 4, 5, 15, 16, 17, len(X[0])-1]
    
        
    #for z in range(72,114):
    #    subset.append(z)
    #X = X[:,subset]
